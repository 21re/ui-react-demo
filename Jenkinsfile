            ...
            withCredentials([usernamePassword(credentialsId: 'nexus-repository', passwordVariable: 'NEXUS_PASSWORD', usernameVariable: 'NEXUS_USER')]) {
                wrap($class: 'AnsiColorBuildWrapper', colorMapName: 'xterm') {
                    sh """
                        ...
                        docker run --rm --user 1000:1000 --group-add 999 -e npm_config_email=admin@21re.de -e npm_config_auth=\$NPM_AUTH -v \$HOME/workspace/.yarn-cache:/home/jenkins/.yarn-cache -v \$(pwd):/project -v \$HOME/workspace/.npm:/home/jenkins/.npm 351075005187.dkr.ecr.eu-west-1.amazonaws.com/builders:play-scala-2 npm publish
                        """
                }
            }
        }
